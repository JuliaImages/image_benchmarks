...
function time_generics(workdir)
    tdata = Dict(task=>Dict{String,Float64}() for task in ["complement", "mean", "gradient", "blur", "histeq", "corners_harris", "corners_shitomasi", "corners_kr", "corner_fastcorners"])
...
function time_generics!(tdata, workdir, taskitems)
    @showprogress "Timing generic operations: " for fn in readdir(workdir)
        ...
        # imagecorners.jl
        "corners_harris" ∈ taskitems     && (tdata["corners_harris"][fn] = @belapsed run_corners_harris($img) samples=16 evals=1)
        "corners_shitomasi" ∈ taskitems  && (tdata["corners_shitomasi"][fn] = @belapsed run_corners_shitomasi($img) samples=16 evals=1)
        "corners_kr" ∈ taskitems         && (tdata["corners_kr"][fn] = @belapsed run_corners_kr($img) samples=16 evals=1)
        "corners_fastcorners" ∈ taskitems && (tdata["corners_fastcorners"][fn] = @belapsed run_corners_fastcorners($img) samples=16 evals=1)
    end
    return tdata
end
