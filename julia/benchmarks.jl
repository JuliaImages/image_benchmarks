...
run_blur(img) = imfilter(img, KernelFactors.gaussian(ntuple(_->5, ndims(img))))
...
function time_generics(workdir)
    ...
    @showprogress "Timing generic operations: " for fn in readdir(workdir)
        ...
        tasks["blur"][fn] = @belapsed run_blur_iir($img) samples=16 evals=1
